...
INT IsImage(LPCTSTR file)
{
	...
	if((dwSign & 0x0000FFFF) == SIGN_BMP)
		res = IMAGE_BMP;
	if((dwSignJpeg & 0xFFFF) == (SIGN_JPG & 0xFFFF)) //из-за того что некоторые файлы дополнены нулями
		res = IMAGE_JPG;
	return res;
}
...
extern LPCTSTR error_reasons[] = {	_T("Не найден ресурс: "),
									_T("Нет доступа к ресурсу"),
									_T("Невозможно создать файл: "),
									_T("Невозможно записать файл: "),
									_T("Операция прервана пользователем.")};
